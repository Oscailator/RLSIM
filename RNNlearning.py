...
class RNN(nn.Module):
    ...
    def forward(self, x, h_state):
        ...
        outs = []
        for time_step in range(r_out.size(1)-UPDATE_STEP,r_out.size(1)): # 按照time_step将输出放入
...
optimizer = torch.optim.RMSprop(rnn1.parameters(), lr=LR, alpha=0.99, eps=1e-08, weight_decay=0, momentum=0, centered=False)
